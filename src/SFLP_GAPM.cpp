...
double SFLP_GAPM::compute_UB(vector<solution_sps> &sp_info){
    double UB_1 = 0.0;
    for (size_t i = 0; i < nFacilities; i++){
    ...
    for (size_t s = 0; s < nScenarios; s++){
        UB_1 += probability[s] * sp_info[s].obj;
    }
    return UB_1;
}
...
void SFLP_GAPM::SPProblemModification_GRB(vector<size_t> &element, bool mod_x) {
    ...
    if (mod_x == true) {
        for (size_t i = 0; i < nFacilities; i++) {
            ...
            //Modify the RHS
            subprob_grb.set(GRB_DoubleAttr_RHS, facil_capacities[i]*x_bar[i]);
        }
    }
    //Updating the model
    subprob_grb.update();
...
void SFLP_GAPM::SPProbleSolution_GRB(vector<double> &stoch, solution_sps &sp_info) {
    ...
    size_t F =0;
    F = subprob_grb.get(GRB_IntAttr_Status);
